...
//           Comprehensive ERP solutions tailored to your business needs, including Sales & Purchase, CRM, Help Desk, HRMS, AMS, CTR, Project and Material Tracking, and more.
//         </p>
//       </div>
...
//       {/* Service Cards - Below principles, compact grid for all active service cards */}
...
//               <div key={serviceCard._id} className="bg-white/90 dark:bg-neutral-900/90 backdrop-blur-sm rounded-2xl shadow-xl p-6 lg:p-8 border border-white/20 hover:border-blue-400/50 hover:shadow-2xl transition-all duration-500" style={{ animationDelay: `${idx * 150}ms` }}>
...
//                       {serviceCard.features.map((feature, featureIdx) => (
...
export default function Services() {
  ...
  return (
    <section className="w-full flex flex-col items-center px-4 pt-16 pb-24 min-h-screen bg-gradient-to-br from-[#e0e7ff] via-[#f0f4ff] to-[#c7d2fe] dark:from-[#0a0a23] dark:via-[#1e293b] dark:to-[#0a0a23] overflow-hidden">
      ...
      {/* Services Grid - Enhanced cards with images, glows, and hovers - Now 3 columns on lg */}
      <div className="max-w-7xl w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 mb-24 animate-fade-in-up animation-delay-200">
        {services.map((service, idx) => (
          <div
            key={service._id}
            className="group relative bg-white/95 dark:bg-neutral-900/95 backdrop-blur-sm rounded-2xl shadow-xl overflow-hidden border border-white/20 hover:border-blue-400/70 hover:shadow-[0_0_20px_rgba(59,130,246,0.3)] hover:shadow-blue-500/20 hover:-translate-y-2 transition-all duration-700 cursor-pointer"
            style={{ animationDelay: `${idx * 150}ms` }}
          >
            <div className="absolute inset-0 bg-gradient-to-br from-blue-900/10 to-purple-900/10 opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
            <img 
            ...
            <div className="relative z-10 p-6 flex flex-col items-center flex-grow">
              <div className="w-10 h-10 mb-3 bg-gradient-to-br from-blue-100 to-purple-100 dark:from-blue-900/30 dark:to-purple-900/30 rounded-full flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                <BoltIcon className="w-5 h-5 text-blue-700 dark:text-blue-400" />
              </div>
              <h3 className="font-bold text-lg mb-3 text-blue-700 dark:text-blue-400 group-hover:underline underline-offset-4 text-center leading-tight">
                {service.title}
              </h3>
              <p className="text-neutral-700 dark:text-neutral-300 text-sm mb-4 text-center leading-relaxed opacity-0 group-hover:opacity-100 transition-opacity duration-500 line-clamp-2">
                {service.description}
              </p>
              <button 
                onClick={() => openModal(service)}
                className="group/btn inline-flex items-center gap-2 mt-auto text-blue-700 dark:text-blue-400 font-semibold hover:text-blue-900 dark:hover:text-blue-300 transition-colors duration-300 underline underline-offset-2 text-sm"
              >
                View More
