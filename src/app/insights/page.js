...
export default function Insights() {
  ...
  return (
    <section className="w-full flex flex-col items-center px-4 pt-16 pb-24 min-h-screen bg-gradient-to-br from-[#e0e7ff] via-[#f0f4ff] to-[#c7d2fe] dark:from-[#0a0a23] dark:via-[#1e293b] dark:to-[#0a0a23] overflow-hidden">
      ...
      {/* Insight Cards - Compact grid, below featured reads */}
      {insightCards.length > 0 && (
        <div className="max-w-7xl w-full mb-16 animate-fade-in-up animation-delay-800">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {insightCards.map((insightCard, idx) => (
              <div key={insightCard._id} className="bg-white/90 dark:bg-neutral-900/90 backdrop-blur-sm rounded-2xl shadow-xl p-6 lg:p-8 border border-white/20 hover:border-blue-400/50 hover:shadow-2xl transition-all duration-500" style={{ animationDelay: `${idx * 150}ms` }}>
                <div className="text-center">
                  <h2 className="text-2xl font-bold mb-4 text-blue-700 dark:text-blue-400">{insightCard.title}</h2>
                  <p className="text-neutral-700 dark:text-neutral-300 mb-6 text-base leading-relaxed max-w-3xl mx-auto">{insightCard.description}</p>
                  {insightCard.features?.length > 0 && (
                    <div className="grid grid-cols-1 gap-4 mb-6">
                      {insightCard.features.map((feature, featureIdx) => (
